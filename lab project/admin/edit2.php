...
<div class='container' >
    ...
    <?php
            ...
            $stu_id = $_GET['id'];
            $sql = "SELECT * FROM merit where ID = {$stu_id}";
            $result = mysqli_query($conn, $sql) or die("Query unsuccessful");
    if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_assoc($result)){
        ?>
    <form class="post-form" action="update2.php" method="post">
      ...
      <div class="form-group">
          ...
          <input type="number" class="form-control" min = "0" max="1100" name="oMarks" value="<?php echo $row['FSc_Marks'];?>"/>
      </div>
      ...
      <div class="form-group">
                <button type="submit" class="button-84" name="update">Update</button>
